...
model1 <- lm(TARGET_WINS ~ TEAM_BATTING_H + TEAM_BATTING_HR + TEAM_BATTING_BB + TEAM_BASERUN_SB + TEAM_FIELDING_E, data = train_data)
summary(model1)

# Model 2: Adding Interaction Terms
model2 <- lm(TARGET_WINS ~ TEAM_BATTING_H * TEAM_BATTING_HR + TEAM_BATTING_BB + TEAM_BASERUN_SB + TEAM_FIELDING_E, data = train_data)
summary(model2)

# Model 3: Applying Log Transformation to Key Predictors
train_data <- train_data %>% mutate(log_TEAM_BATTING_H = log(TEAM_BATTING_H + 1), log_TEAM_BATTING_HR = log(TEAM_BATTING_HR + 1))
model3 <- lm(TARGET_WINS ~ log_TEAM_BATTING_H + log_TEAM_BATTING_HR + TEAM_BATTING_BB + TEAM_BASERUN_SB + TEAM_FIELDING_E, data = train_data)
summary(model3)

# Model 4: Incorporating Pitching and Defensive Metrics
model4 <- lm(TARGET_WINS ~ TEAM_BATTING_H + TEAM_BATTING_HR + TEAM_PITCHING_SO + TEAM_PITCHING_BB + TEAM_FIELDING_E, data = train_data)
summary(model4)

# Model 5: Including Caught Stealing and Doubles
model5 <- lm(TARGET_WINS ~ TEAM_BATTING_H + TEAM_BATTING_HR + TEAM_BATTING_2B + TEAM_BASERUN_CS + TEAM_FIELDING_E, data = train_data)
summary(model5)

# Model 6: Weighted Batting and Pitching Metrics
model6 <- lm(TARGET_WINS ~ (TEAM_BATTING_H + TEAM_BATTING_BB) / TEAM_BATTING_SO + (TEAM_PITCHING_SO - TEAM_PITCHING_BB), data = train_data)
summary(model6)
```
```{r}
...
eval_results <- lapply(models, model_eval, data = train_data)
