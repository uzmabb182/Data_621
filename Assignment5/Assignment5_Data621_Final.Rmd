...
Dropping our negative values and imputing our missing values introduced skewness into most of the numerical variables. All variables but Alcohol, Density and pH have are right—skewed  --though the right tail for FixedAcidity and TotalSulfurDioxide are relatively moderal compared the heavy right tails of the other variables.
...
for (colname in names(wine_training_imputed)[numeric_cols]) {
  ...
  if (is.numeric(wine_training_imputed[[colname]]) && !is.na(colname) && colname != "TARGET") {
...
We can apply log transformation to these variables to help address skewness.
...
```{r distributions3, fig.width=7, fig.height=5, echo=FALSE, message = FALSE, warning=FALSE}
...
for (colname in names(wine_training_clean)[numeric_cols]) {
  ...
  if (is.numeric(wine_training_clean[[colname]]) && !is.na(colname) && colname != "TARGET") {
