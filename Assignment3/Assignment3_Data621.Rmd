...
### Computing Prediction Error and CV Error:

These values represent the Adjusted Cross-Validation Error for each model — the lower, the better.

```{r, warning = FALSE, message = FALSE}
library(boot)
# CV for Model 1
cv_model1 <- cv.glm(data = crime_training_df, glmfit = model1, K = 10)
cv_error_model1 <- cv_model1$delta[2]  # Adjusted CV Error

# CV for Model 2
cv_model2 <- cv.glm(data = crime_training_df, glmfit = model2, K = 10)
cv_error_model2 <- cv_model2$delta[2]

# CV for Model 3
cv_model3 <- cv.glm(data = crime_training_df, glmfit = model3, K = 10)
cv_error_model3 <- cv_model3$delta[2]
...
library(pscl)
# Final Summary Table
model_metrics_final <- tibble::tibble(
  Model = c("Model 1", "Model 2", "Model 3"),
  AIC = c(AIC(model1), AIC(model2), AIC(model3)),
  BIC = c(BIC(model1), BIC(model2), BIC(model3)),
  McFadden_R2 = c(pR2(model1)["McFadden"],
                  pR2(model2)["McFadden"],
                  pR2(model3)["McFadden"]),
  Accuracy = c(0.9334764, 0.9270386, 0.8519313),
  F1_Score = c(0.9318681, 0.9251101, 0.8463252),
  AUC = c(0.9787187, 0.9775579, 0.9472297),
  Prediction_Error = c(1 - 0.9334764, 1 - 0.9270386, 1 - 0.8519313),
  Adjusted_CV_Error = c(cv_error_model1, cv_error_model2, cv_error_model3)
)
