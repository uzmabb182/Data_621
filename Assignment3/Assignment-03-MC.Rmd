...
df_training <- read_csv("crime-training-data_modified.csv")
...
# only show means for numeric values
df_training_numeric <- df_training |>
  subset(select = -c(chas, rad, target))

for (param in colnames(df_training_numeric)) {
  cat("\nSummary for", param, ":\n")
  print(describe(df_training_numeric[[param]]))
}
```
...
```{r data-exploration-simple}
plot_prcomp(na.omit(df_training))
```
...
```{r check-for-correlation, echo=FALSE}
...
probabilities_full <- predict(model_full, type = "response")

# df_training_one_hot_numerical <- df_training_one_hot 
# |>   subset(select = c(age, dis, indus, lstat, medv, nox, ptratio, rm, tax, zn))

predictors <- colnames(df_training_one_hot)

df_training_one_hot_full <- df_training_one_hot |>
  mutate(logit = log(probabilities_full/(1-probabilities_full))) |>
  gather(key = "predictors", value = "predictor_value", -logit)

#for (param in colnames(df_training_one_hot)) {
  
  #df <- df_training_one_hot_full |>
  #s  filter(param %in% predictors)
  
  param_plot <- ggplot(df_training_one_hot, aes(logit, predictor_value))+
    geom_point(size = 0.05, alpha = 0.5) +
    geom_smooth(method = "loess") + 
    theme_bw() + 
    facet_wrap(~predictors, scales = "free_y", ncol=3) + 
    labs(title = paste("Linearity Tests")) +
