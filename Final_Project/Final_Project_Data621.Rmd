...
data$Productivity_Change <- factor(data$Productivity_Change, 
                                   levels = c("Decrease", "No Change", "Increase"), ordered = TRUE)
...
```{r}
dummy_vars <- dummyVars(Satisfaction_with_Remote_Work ~ ., data = data)
data_dummies <- data.frame(predict(dummy_vars, newdata = data))

data_dummies$Satisfaction_with_Remote_Work <- data$Satisfaction_with_Remote_Work

predictor_names <- setdiff(names(data_dummies), "Satisfaction_with_Remote_Work")
num_cols <- sapply(data_dummies[, predictor_names], is.numeric)

preProcValues <- preProcess(data_dummies[, predictor_names][, num_cols], method = c("center", "scale"))
data_dummies[, predictor_names][, num_cols] <- predict(preProcValues, data_dummies[, predictor_names][, num_cols])
...
```{r}
set.seed(621)  
trainIndex <- createDataPartition(data_processed$Satisfaction_with_Remote_Work, 
                                  p = 0.7,  
                                  list = FALSE)
