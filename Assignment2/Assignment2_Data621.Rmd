...
Accuracy= (27+119+5+30)/27+119= 146/181=0.8066(80.66%)
...
```
### Function to Generate ROC Curve and Compute AUC
...
generate_roc_curve <- function(data) {
  ...
  return(list("ROC Plot" = roc_curve, "AUC" = auc_value))
}
sum(is.na(classification_df$scored.probability))
classification_df <- na.omit(classification_df)  # Remove rows with NA values
# Compute ROC and AUC
roc_results <- generate_roc_curve(classification_df)
...
### Compute and Print AUC Score Using pROC
...
# Compute AUC Score
auc_value <- as.numeric(auc(roc_curve))  # Convert to numeric to avoid type issues
...
### Creating Graphical Outputs for Evaluating Classification Models

### Generate ROC Curve with AUC (Receiver Operating Characteristic Curve)
...
X-axis (Specificity) shows True Negative Rate (1 - False Positive Rate).
Y-axis (Sensitivity) shows True Positive Rate (Recall).
The blue curve represents how the model distinguishes between positive and negative classes at different thresholds.
The red dashed line represents a random classifier (AUC = 0.5), meaning no predictive power.
...
```
The ROC Curve Shape (Blue Line)
The blue ROC curve shows that your model performs better than random guessing (diagonal line).
The curve bends toward the top-left corner, meaning the model is good at distinguishing between positive and negative cases.
The Red Diagonal Line (Baseline)
The red dashed line (45-degree line) represents a classifier that randomly predicts classes (AUC = 0.5).
Your ROC curve is well above this line, which indicates a strong classification ability.

"AUC Score: 0.850311262026033"
