...
# Define function to generate ROC Curve and compute AUC Score
generate_roc_curve <- function(data, actual_col, prob_col) {
  # Ensure actual class column is a factor (required for pROC)
  ...
  roc_curve <- roc(response = data[[actual_col]], predictor = data[[prob_col]], 
                   percent = FALSE, ci = TRUE, smooth = TRUE)

  # Plot the ROC Curve
  plot(roc_curve, col = "blue", main = "ROC Curve", print.thres = seq(0, 1, 0.01))
