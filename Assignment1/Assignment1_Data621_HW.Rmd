...
### Model 4: Variabls Selection Using Backward Selection 
...
Before starting, we dropped the _TEAM_BATTING_2B_ (2 base hits), _TEAM_BATTING_3B_ (3 base hits), _TEAM_BATTING_HR_ (home runs) as data for these columns is represented in the _TEAM_BATTING_H_ (total hits) column, thus signaling the existence of dependence between hit count variables and total hits. 
...
train_df2 <- drop_columns(train_df_clean, c("TEAM_BATTING_2B", "TEAM_BATTING_3B", "TEAM_BATTING_HR", "TEAM_PITCHING_HR", "TEAM_BASERUN_SB_Missing"))
...
```{r}
library(leaps)
regfit_full = regsubsets(TARGET_WINS ~ ., data = train_df2)
summary(regfit_full)
```
