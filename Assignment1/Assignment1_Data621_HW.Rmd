...
model1 <- lm(TARGET_WINS ~ TEAM_BATTING_H + TEAM_BATTING_HR + TEAM_BATTING_BB + TEAM_BASERUN_SB + TEAM_FIELDING_E, data = stp75_train_df)
summary(model1)
...
stplg75_train_df_2 <- stp75_train_df %>% mutate(log_TEAM_BATTING_H = log(TEAM_BATTING_H + 1), log_TEAM_BATTING_HR = log(TEAM_BATTING_HR + 1))
model3 <- lm(TARGET_WINS ~ log_TEAM_BATTING_H + log_TEAM_BATTING_HR, data = stplg75_train_df_2)
summary(model3)

# Model 4: Incorporating Pitching and Defensive Metrics
model4 <- lm(TARGET_WINS ~ TEAM_BATTING_H + TEAM_BATTING_HR + TEAM_PITCHING_SO + TEAM_PITCHING_BB + TEAM_FIELDING_E, data = stp75_train_df)
summary(model4)

# Model 5: Including Caught Stealing and Doubles
model5 <- lm(TARGET_WINS ~ TEAM_BATTING_H + TEAM_BATTING_HR + TEAM_BATTING_2B + TEAM_BASERUN_CS + TEAM_FIELDING_E, data = stp75_train_df)
summary(model5)
...
# Model 3: Applying Log Transformation to Key Predictors
stplg75_train_df_2 <- stp75_train_df %>% mutate(log_TEAM_BATTING_H = log(TEAM_BATTING_H + 1), log_TEAM_BATTING_HR = log(TEAM_BATTING_HR + 1))
model3 <- lm(TARGET_WINS ~ log_TEAM_BATTING_H + log_TEAM_BATTING_HR, data = train_data_log)
summary(model3)
