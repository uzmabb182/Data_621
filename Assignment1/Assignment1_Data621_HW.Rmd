...
- TEAM_BATTING_SO & TEAM_PITCHING_SO have the same rows entered as zero suggesting that data may not have been available for these entries.
- TEAM_BATTING_HR & TEAM_PITCHING_HR have the same rows entered as zero suggesting that data may not have been available for these entries.
...
```{r correlation-before-imputation}
...
_Top 3 Positive Correlation_
```{r}
head(cor_df, 3)
```

_Top 3 Negative Correlation_
```{r}
...
eval_predictions(base_model, stp25_test_df$PREDICTED_WINS_INV, stp25_test_df$TARGET_WINS, "Base Model (Back-Transformed)")
...
stp25_test_df$log_TEAM_BATTING_H <- log(stp25_test_df$TEAM_BATTING_H + 1) 
...
cat("MAE: ", lasso_metrics$MAE, "\n")
cat("RMSE: ", lasso_metrics$RMSE, "\n")
cat("R-squared: ", lasso_metrics$RSquared, "\n")
cat("AIC: ", lasso_metrics$AIC, "\n")
cat("BIC: ", lasso_metrics$BIC, "\n")
...
# Interpretation
cat("\nInterpretation:\n")
cat("The Lasso model achieved an MAE of", lasso_metrics$MAE, 
    "which suggests the model has a low average error in predicting TARGET_WINS. The RMSE value of", 
    lasso_metrics$RMSE, "supports this conclusion. The R-squared value of", lasso_metrics$RSquared, 
    "indicates that the Lasso model explains a substantial portion of the variance in the TARGET_WINS.\n")
...
mean(eval_df$PREDICTED_WINS)    
sd(eval_df$PREDICTED_WINS)  

```

```{r}
plot(density(eval_df$PREDICTED_WINS), main = "Density of Predicted Wins", 
     xlab = "Predicted Wins", col = "blue")  
```
