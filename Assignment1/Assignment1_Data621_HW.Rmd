...
Multivariate Imputation by Chained Equations (MICE) is a type of Multiple Imputation. MICE accounts for the relationships between different variables, which helps provide more accurate imputations. MICE assumes that data are Missing at Random (MAR); if data is not Missing at Random (MAR), we can inadvertently introduce bias in our data.
...
train_df_mice <- mice(data=train_df_median, m=30, maxit=10, seed=12345, print=FALSE)
...
### Model 4: Alternate Backward Variable Selection
...
### 4B: Model with Transformed Data
...
#### 4Bi: Box-Cox Transformation on Dependent Variable
...
#### 4Bii: Log Transformation on the Dependent Variable (Y)
...
TODO: update based on updated  models
...
# Model 1: Basic Multiple Linear Regression with Key Predictors
...
model1 <- lm(TARGET_WINS ~ ., data = stp75_train_df[, c("TARGET_WINS", target_vars1)])
summary(model1)
...
model2 <- lm(TARGET_WINS ~ TEAM_BATTING_H * TEAM_BATTING_HR + TEAM_BATTING_BB + TEAM_BASERUN_SB + TEAM_FIELDING_E, data = stp75_train_df)
summary(model2)

# Model 3: Applying Log Transformation to Key Predictors
# Transformation applied to account for skewed distributions in key predictors
stp75_train_df <- stp75_train_df %>% 
  mutate(log_TEAM_BATTING_H = log(TEAM_BATTING_H + 1), log_TEAM_BATTING_HR = log(TEAM_BATTING_HR + 1))
target_vars3 <- c("log_TEAM_BATTING_H", "log_TEAM_BATTING_HR", "TEAM_BATTING_BB", "TEAM_BASERUN_SB", "TEAM_FIELDING_E")
model3 <- lm(TARGET_WINS ~ ., data = stp75_train_df[, c("TARGET_WINS", target_vars3)])
summary(model3)

# Model 4: Incorporating Pitching and Defensive Metrics
...
model4 <- lm(TARGET_WINS ~ ., data = stp75_train_df[, c("TARGET_WINS", target_vars4)])
summary(model4)

# Model 5: Including Caught Stealing and Doubles
...
model5 <- lm(TARGET_WINS ~ ., data = stp75_train_df[, c("TARGET_WINS", target_vars5)])
summary(model5)

# Model 6: Weighted Batting and Pitching Metrics
# Creating composite metric for batting and pitching efficiency
model6 <- lm(TARGET_WINS ~ (TEAM_BATTING_H + TEAM_BATTING_BB) / TEAM_BATTING_SO + (TEAM_PITCHING_SO - TEAM_PITCHING_BB), data = stp75_train_df)
summary(model6)
...
for (i in 1:length(models)) {
  cat("\n", model_names[i], "\n")
  print(AIC(models[[i]]))  # Akaike Information Criterion
  print(BIC(models[[i]]))  # Bayesian Information Criterion
}

# Model Evaluation
model_eval <- function(model, data) {
  predictions <- predict(model, data)
  mse <- mse(data$TARGET_WINS, predictions)
  r2 <- summary(model)$r.squared
  return(list(MSE = mse, R2 = r2))
}

eval_results <- lapply(models, model_eval, data = stp75_train_df)
names(eval_results) <- model_names
eval_results

# Testing Assumptions of Linear Regression
par(mfrow = c(2, 2))
for (i in 1:length(models)) {
  plot(models[[i]], main = paste("Assumption Checks -", model_names[i]))
}

# Applying the same log transformation to the evaluation dataset
eval_df <- eval_df %>%
  mutate(log_TEAM_BATTING_H = log(TEAM_BATTING_H + 1),
         log_TEAM_BATTING_HR = log(TEAM_BATTING_HR + 1))

# Using the best model (assumed Model 3 for now) to make predictions
predictedWins <- predict(model3, eval_df, na.action = na.pass)
...
stp75_train_df <- train_df[train_ind, ]  # 75% training data
stp25_test_df <- train_df[-train_ind, ]  # 25% testing data
...
#TODO: these should also be compared to the old models
...
eval_results <- lapply(models, model_eval, data = stplg75_train_df_2)
names(eval_results) <- model_names
eval_results
```
...
```{r}
model_performance <- compare_performance(base_model, high_impact_model, log_model, 
                                         back_select_model, stp_model_sm, stp_logx_model, 
                                         lasso_model, rank = TRUE)

# Computes Elastic Net performance
elastic_net_predictions <- predict(elastic_net_model, newx = x_test, s = best_lambda)
elastic_net_mae <- mae(y_test, elastic_net_predictions)
elastic_net_rmse <- rmse(y_test, elastic_net_predictions)
elastic_net_r2 <- cor(y_test, elastic_net_predictions)^2

# Computes Random Forest performance
random_forest_predictions <- predict(random_forest_model, newdata = stp25_test_df)
random_forest_mae <- mae(y_test, random_forest_predictions)
random_forest_rmse <- rmse(y_test, random_forest_predictions)
random_forest_r2 <- cor(y_test, random_forest_predictions)^2

# Stores all results in a data frame
model_metrics <- data.frame(
  model_name = c("Elastic Net", "Random Forest"),
  MAE = c(elastic_net_mae, random_forest_mae),
...
selected_model <- stp_logx_model
formula <- formula(selected_model$finalModel) 
...
mean_val <- colMeans(eval_df, na.rm = TRUE)

# Impute using Means
#for(i in colnames(eval_df))
#    eval_df[,i][is.na(eval_df[,i])] <- mean_val[i]
```
