...
for(i in colnames(train_df_clean))
    train_df_median[,i][is.na(train_df_median[,i])] <- as.integer(train_median_val[i])
...
Backward selection using P-values arrives at a model with eight variables with high statistical significance  ((p-value < 0.001) and three with moderate (p-value ~= 0.1) statistical significance. Arriving at 11 predictors is consistent with our Best Subset Selection test.
...
#### Tuning with Transformations

```{r}
library(MASS)
stpbxcx_model <- boxcox(stp_model)
plot(stpbxcx_model)
```
...
stplogy75_train_df <- stp75_train_df %>%
  mutate(
    TARGET_WINS = log1p(TARGET_WINS)
  )

stp_logy_model <- lm(TARGET_WINS ~., data = stplogy75_train_df)

# Backward step-wise regression
stpb_logy_model <- step(stp_logy_model, direction = "backward")

plot(stpb_logy_model)
```
