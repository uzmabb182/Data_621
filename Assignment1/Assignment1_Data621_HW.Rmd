...
Of the remaining variables, _TEAM_BATTING_SO_ and _TEAM_PITCHING_SO_ included a several observations with a value of zero. As the rows were the same for both variables, it appears that these may also be missing observations as the likelihood that a team's batters did not have a single strikeout nor did their pitchers pitch a single strikeout over the course of a 162 game season is highly unlikely. We will therefore treat these as missing observations and impute using the median value. We will also drop rows where the team did not win a single game, as these too look dubious.
...
for(k in 1:5) {
  best.fit = regsubsets(TARGET_WINS ~ ., data=stp75_train_df[folds!=k,], nvmax=10, method="forward")
  for(i in 1:10) {
    ...
    cv.errors[k,i]=mean((stp75_train_df$TARGET_WINS[folds==k] - pred)^2)
  }
}
...
```{r}
stp_model2 <- update(stp_model, . ~ . - TEAM_PITCHING_BB)
summary(stp_model2)
AIC(stp_model2)
```
...
```{r}
stp_model2 <- update(stp_model2, . ~ . - TEAM_BATTING_SO)
summary(stp_model2)
AIC(stp_model2)
```

_TEAM_BASERUN_CS_ has the highest p-value; since this p-value is above 0.05, it may not be statistically significant. We remove TEAM_BASERUN_CS from our predictors and update the model.


```{r}
stp_model2 <- update(stp_model2, . ~ . - TEAM_BASERUN_CS)
summary(stp_model2)
AIC(stp_model2)

```

Backward selection using P-values arrives at a model with seven variables of statistical significance — six strong (< 0.001) and one moderate (0.05). 
...
#### Model 4 with Log Transformations
