...
For this model, we are using backward selection to arrive at the smallest number of variables with statistical significance. In this case, we begin by creating a model containing all
predictors, then gradually remove the variable with the highest P-value until only the variables with statistical significance remain. 

Before starting, we dropped the _TEAM_BATTING_2B_ (2 base hits), _TEAM_BATTING_3B_ (3 base hits), _TEAM_BATTING_HR_ (home runs) as data for these columns is represented in the _TEAM_BATTING_H_ (total hits) column, thus signaling the existence of dependence between hit count variables and total hits. It is not totally clear if _TEAM_PITCHING_HR_ is also included in _TEAM_PITCHING_H_ from the supporting document but we will remove it in the event that a similar relationship exists. __Alternatively__, we could have created a new column (TEAM_BATTING_1B) by subtracting the sum of 2 base hits, 3 base hits, and homeruns from total hits, but this alternatively solution would have a) added complexity into our model and b) added uncertainty due to possible missing values in the homeruns columns. 



```{r model4-transformations}
stp_train_df <- train_df_clean |> 
    drop_columns(c("TEAM_BATTING_2B", "TEAM_BATTING_3B", "TEAM_BATTING_HR", "TEAM_PITCHING_HR", "TEAM_BASERUN_SB_Missing"))
```

Of the remaining variables, _TEAM_BATTING_SO_ and _TEAM_PITCHING_SO_ included a several observations with a value of zero. As the rows were the same for both variables, it appears that these may also be missing observations as the likelihood that a team's batters did not have a single strikeout nor did their pitchers pitch a single strikeout over the course of a 162 game season is highly unlikely. We will therefore treat these as missing observations and impute using the median value. We will also drop the single row where the team did not win a single game, as this is also suspicious.
...
# get means
train_median_val <- colMedians(stp_train_df, na.rm = TRUE)

# Impute using Means
for(i in colnames(stp_train_df))
    stp_train_df[,i][is.na(stp_train_df[,i])] <- as.integer(train_median_val[i])

sum(is.na(stp_train_df))  # Total missing values
colSums(is.na(stp_train_df))  # Missing values per column

```
The scatterplots look a bit better after making these adjustments, but we may need to apply a log  transformation to address outliers seen in some of our plots.

```{r model4-distributions}
...
#plot_qq(stp_train_df, sampled_rows = 1000L)
...
We will split the original training dataset into a training and testing dataset in order to test the strength of our model without testing the model on data that the model has already seen. The training dataset will hold 75% of our original observations, while the test dataset will hold 25%. 

```{r model4-split-data}

smp_size <- floor(0.75 * nrow(stp_train_df))

## set the seed to make your partition reproducible
set.seed(123)
train_ind <- sample(seq_len(nrow(stp_train_df)), size = smp_size)

stp75_train_df <- stp_train_df[train_ind, ]
stp25_test_df <- stp_train_df[-train_ind, ]
```
...
```
Based on the _Best Subset Selection_ method, we estimate that our model should have `r which.min(rmse_cv)` observations.

##### Backward Selection Method

```{r backward-selection}
stp_model <- lm(TARGET_WINS ~ ., data = stp75_train_df)
summary(stp_model)
AIC(stp_model)
```
...
```{r}
stp_model2 <- update(stp_model, . ~ . - TEAM_BATTING_SO)
summary(stp_model2)
AIC(stp_model2)
```
...
```{r}
stp_model_ph <- update(stp_model2, . ~ . - TEAM_PITCHING_BB)
summary(stp_model_ph)
AIC(stp_model_ph)
```

_TEAM_PITCHING_H_ has the highest p-value; since this p-value is above 0.05, it may not be statistically significant. We remove TEAM_PITCHING_H from our predictors and update the model.


```{r}
stp_model_no_ph <- update(stp_model_ph, . ~ . - TEAM_PITCHING_H)
summary(stp_model_no_ph)
AIC(stp_model_no_ph)

```

Backward selection using P-values arrives at a model with seven variables of statistical significance — six strong ((p-value < 0.001) and one moderate (p-value ~= 0.1). However, our (AIC) value has increased from `r AIC(stp_model2)` to `r AIC(stp_model3)` suggesting that we may not want to remove TEAM_PITCHING_H from our model. Having 7-8 predictors is consistent with our Best Subset Selection test.

Performing a variance inflation factor (VIF) test on the larger model (8 predictors including TEAM_PITCHING_H)  shows only moderate correlation between the predictors and the dependent variable.

```{r model-4-diagnostic-w_ph plots}
vif(stp_model_ph)
plot(stp_model_ph)
```

Performing a variance inflation factor (VIF) test on the smaller model (7 predictors not including TEAM_PITCHING_H) shows even smaller correlation between the predictors and the dependent variable. As a result, we may prefer the simpler model.

```{r model-4-diagnostic-wo_ph plots}
vif(stp_model_no_ph)
plot(stp_model_no_ph)
```
##### Testing the model

```{r model4-test}

predictedWins = predict(stp_model_no_ph, stp25_test_df)
stp25_test_df["PREDICTED_WINS"] = predictedWins

stp25_test_df <- stp25_test_df %>% 
  relocate("PREDICTED_WINS", .after = "TARGET_WINS")  %>% 
  mutate(
    PREDICT_ERR = TARGET_WINS - PREDICTED_WINS
  )  %>% 
  relocate("PREDICT_ERR", .after = "PREDICTED_WINS")

pred = lm(stp25_test_df$PREDICTED_WINS~stp25_test_df$TARGET_WINS)

plot(stp25_test_df$PREDICTED_WINS, stp25_test_df$TARGET_WINS,xlab="Actual Wins", ylab="Predicted Wins")
abline(pred)
summary(pred)
```
...
stplg_train_df <- stp_train_df %>%
  #filter(TEAM_PITCHING_SO < 10000) %>%
  mutate(
...
#plot_qq(stplg_train_df, sampled_rows = 1000L)

```

```{r model4lg-split-data}

smp_size <- floor(0.75 * nrow(stplg_train_df))

## set the seed to make your partition reproducible
set.seed(123)
train_ind <- sample(seq_len(nrow(stplg_train_df)), size = smp_size)

stplg75_train_df <- stplg_train_df[train_ind, ]
stplg25_test_df <- stplg_train_df[-train_ind, ]
```
...
We get the same number of observations after testing for Best Subset Selection & Cross-Validation, with `r which.min(regfit_summary$cp)` and `r which.min(rmse_cv)` observations respectively.
...
predictedWins = predict(stp_model_no_ph, stplg25_test_df)
stplg25_test_df["PREDICTED_WINS"] = predictedWins

stplg25_test_df <- stplg25_test_df %>% 
  relocate("PREDICTED_WINS", .after = "TARGET_WINS")  %>% 
  mutate(
    PREDICT_ERR = TARGET_WINS - PREDICTED_WINS
  )  %>% 
...
pred = lm(stplg25_test_df$PREDICTED_WINS~stplg25_test_df$TARGET_WINS)

plot(stplg25_test_df$PREDICTED_WINS, stplg25_test_df$TARGET_WINS,xlab="Actual Wins", ylab="Predicted Wins")
abline(pred)
summary(pred)
```
...
```{r}
library(glmnet)
x = model.matrix(TARGET_WINS ~ ., data = stp_train_df)
y = stp_train_df$TARGET_WINS
