...
# Compare models using compare_performance
model_comparison <- compare_performance(base_model, high_impact_model, log_model, back_select_model, stp_model_sm, stp_logx_model, lasso_model, model1, model2, model3, model4, model5, model6)
...
```{r}
#TODO: merge this with below so that they can all be evaluated at once
...
```{r}
# this is a placeholder. we can change this once we decide
final_model <- stp_logx_model
final_df <- stplg75_train_df
```
...
model_cv <- train(formula(selected_model), data = final_df, method = "lm", trControl = train_control)
...
predictedWins = predict(final_model, eval_df)
