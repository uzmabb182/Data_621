...
Each small plot (facet) shows the relationship between one predictor variable and TARGET_WINS.

Data points are plotted where the x-axis represents the predictor variable, and the y-axis represents the number of wins.

A black trend line is fitted using linear regression is showing the general direction of the relationship.

The slope of the regression line in each facet indicates whether a variable has a positive or negative correlation with TARGET_WINS.

If the points are closely clustered around the line, it suggests a stronger linear relationship.

If the points are widely scattered, the variable may not strongly predict TARGET_WINS.
...
Weak or No Influence: INDEX, TEAM_BATTING_3B, TEAM_BATTING_HBP, TEAM_BATTING_SO, TEAM_FIELDING_DP, TEAM_PITCHING_HR.
...
```
Test Statistics W = 0.99682 is close to 1 showing Normality, but 
p-value = 0.0001093 < 0.05 suggest that residuals do not follow a normal distribution
...
```
Test Statistic (BP = 272.22) suggest higher evidence of heteroscedasticity
p-value is extremely small (much less than 0.05) suggest higher evidence of heteroscedasticity
and we may need to transform the data.
...
```
 Autocorrelation = 0.4966624, This value is positive, indicating that residuals are positively correlated
D-W Statistic = 1.003569 < 2.0 suggest Positive autocorrelation (residuals are correlated)
 p-value = 0 < 0.05, suggest there is strong evidence of autocorrelation
...
library(lmtest)
shapiro.test(residuals(stplg_model))
# Shapiro-Wilk normality test: look for high p-value
``` 
Although the W-statistic of 0.9939 is close to 1, the very small p-value (1.76e-06) indicates that there is strong evidence to suggest that the residuals from your model do not follow a normal distribution.

##### Heteroscedastic 

```{r check-for-homoscedastic}
bptest(stplg_model)
# Breusch-Pagan test; look for high p-value
``` 
Since the p-value is much smaller than 0.05, you reject the null hypothesis of no heteroscedasticity. This suggests that there is strong evidence of heteroscedasticity in your model.

##### Independence 

```{r check-for-independence}
durbinWatsonTest(stplg_model)
# Durbin Watson test: should be close to 2
``` 

The D-W statistic of 2.011436 suggests that there is no significant autocorrelation in the residuals of your model. The p-value of 0.84 reinforces this conclusion, as it shows that the evidence against the null hypothesis of no autocorrelation is weak.
...
predictedWins = predict(improved_model2, stp25_test_df)
...
```{r}
# Improved Model
eval_predictions(improved_model2, stp25_test_df$PREDICTED_WINS_IMP, stp25_test_df$TARGET_WINS, "Improved")
```
