...
Since The TARGET_WINS column has missing values, we should remove those rows since we cant predict missing outcomes. Similarly, if it has a zero (0) value, we may also want to drop this row, as it is highly suspicious that a team would have 0 wins in 162 games.
...
5. TEAM_FIELDING_E
For every additional error, a team is expected to lose 0.02 games. More errors directly hurt a teams chances of winning, which makes sense in baseball.

The F-statistic of 127.1 (p < 2.2e-16) means our model is highly statistically significant. This suggests that at least one of our variablessuch as home runs, hits, strikeouts, or errorshas a real impact on predicting wins.
...
Doubles (2B) are a strong indicator of offensive power and often correlate with scoring more runs. If a team doesnt hit home runs, but hits many doubles, it can still score efficiently. 
...
stp25_test_df$PREDICTED_WINS = predictedWins  # Use $ notation for consistency
...
stp25_test_df$PREDICTED_WINS_ELASTIC <- as.numeric(predict(elastic_net_model, newx = x_test, s = best_lambda))

# Random Forest Model predictions
stp25_test_df$PREDICTED_WINS_RF <- predict(random_forest_model, newdata = stp25_test_df)

# Ensure Random Forest model predictions are correctly processed
stp25_test_df$PREDICTED_WINS_RF <- predict(random_forest_model, newdata = stp25_test_df)
...
eval_predictions <- function(model, predict_col, target_col, model_name) {
  ...
  if (is.null(predict_col) || is.null(target_col)) {
...
```{r}
# Now call eval_predictions for each model
...
```{r}
# Sort and display the evaluation metrics in ascending order of MAE
...
```
```{r}
# Final comparison of all models based on performance metrics
...
# Display final comparison sorted by RMSE
final_model_comparison <- performance_df[order(performance_df$RMSE), ]
...
```{r}
# Evaluate Log-Transformed Model2
eval_predictions(log_model2, test_df_log$PREDICTED_WINS_LOG2, test_df_log$TARGET_WINS, "Log-Tranformed 2")
```
...
```{r}
# Evaluate LASSO
eval_predictions(log_model1, test_df_log$PREDICTED_WINS_LASSO, test_df_log$TARGET_WINS, "Lasso")
```
